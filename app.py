...
def load_data():
    if os.path.exists(DB_FILE):
        ...
        # Define columns based on your required tracking points
...
# ==========================================
# 3. SIDEBAR: PROJECT INTAKE FORM (SIMULATING PM/SALES INPUT)
# ==========================================
...
with st.sidebar.form(key="intake_form", clear_on_submit=True):
    p_lang = st.sidebar.text_input("Language Pair *", placeholder="e.g., English -> Japanese")
    p_type = st.sidebar.selectbox("Task Type *", ["Translation", "Editing", "Localization Testing", "Subtitling", "Voice-Over"])
    p_vol = st.sidebar.text_input("Volume (Words / Minutes) *", placeholder="e.g., 5000 words")
    p_deadline = st.sidebar.date_input("Client Deadline *", min_value=datetime.today())
    p_budget = st.sidebar.number_input("Client Budget ($) *", min_value=0.0, step=50.0)
    
    submit_intake = st.sidebar.form_submit_button("Submit to VM Pipeline")
...
if submit_intake:
    if p_lang and p_vol:
        ...
        new_row = {
            ...
            "Assigned Vendor": "Unassigned",  # Blank default for VM allocation
            "Vendor Rate ($)": 0.0,           # Blank default for VM calculation
            "Project Status": "Pipeline Intake" # Base initial status state
        }
        
        # Append data row back to data stores
        ...
        # SIMULATION OF THE OUTLOOK EMAIL ACTION
...
if df.empty:
    ...
    with col_sel:
        selected_id = st.selectbox("Select Project to Modify:", df["Project ID"].unique())
        
    # Isolate relevant project attributes for editing context updates
    ...
    # Action submission switch handling updates execution loop
